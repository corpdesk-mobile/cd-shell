...
import { IShellConfig } from "./CdShell/sys/base";
...
export class Main {
  ...
  private svController = new ControllerService();
  ...
  async run() {
    ...
    //---------------------------------------
    const shellConfig: IShellConfig = await this.svConfig.resolveShellConfig();
  ...
  private async loginAnonUser(): Promise<void> {
    const consumerGuid = this.svSysCache.getConsumerGuid();
    if (!consumerGuid) return;

    const anonUser: UserModel = { userName: "anon", password: "-" };
    const resp = await this.svUser.login(anonUser, consumerGuid);
    if (!resp) return;
  ...
  // ===================================================================
  private async bootstrapUi(shellConfig: IUserShellConfig): Promise<void> {
  ...
  private async applyStartupUiSettings(
    ...
    if (uiConfig.defaultUiSystemId) {
      this.logger.debug(
        "[Main] Activating UI system",
        uiConfig.defaultUiSystemId
      );
    ...
    if (uiConfig.defaultFormVariant) {
      this.logger.debug(
        "[Main] Applying form variant",
        uiConfig.defaultFormVariant
      );
    ...
    // ---------------------------------------
    if (
      typeof (this.svUiSystemLoader as any).reapplyShellLayout === "function"
    ) {
      this.logger.debug(
        "[Main] Reapplying shell layout CSS to restore layout dominance"
      );
    } else {
      this.logger.warn(
        "[Main] reapplyShellLayout() not found on UiSystemLoaderService"
      );
    }
  }
  ...
  private async tryHideSplash() {
    if (!this.splashAnimDone || !this.appReady) return;
    await this.hideSplash();
  }
