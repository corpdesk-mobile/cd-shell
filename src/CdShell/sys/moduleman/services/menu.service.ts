...
export class MenuService {
  ...
  onMenuClick(item: MenuItem, cdToken?: string) {
    ...
    if (item.children && item.children.length > 0) {
      console.debug(
        `[MenuService] Toggling menu node expansion for '${item.label}'`
      );
    ...
    // 🔹 CASE 5: UNKNOWN TYPE
    console.warn(
      `[MenuService] Unhandled menu item type '${item.itemType}' for label '${item.label}'`
    );
  }
