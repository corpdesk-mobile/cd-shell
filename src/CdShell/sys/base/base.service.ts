...
> extends AbstractBaseService<T> {
  ...
  async initSqlite(req: any, res: any) {
    const iMax = 5;
    const i = 1;
    try {
  ...
  validateJsonUpdate<T>(
    ...
    function traversePath(
      ...
      if (Array.isArray(currentInterface) && currentKey === "[0]") {
        ...
        errors.push(
          `Unexpected type at '${currentPath.join(".")}'. Expected object or array.`
        );
        return false;
      }
    }
  ...
  async redisInit(req: any, res: any) {
  ...
  async redisCreate(req: any, res: any) {
  ...
  async handleRequest<T = any>(
    ...
    try {
      ...
      if (!cdResp.app_state.success) {
        const msg =
          cdResp.app_state.info?.app_msg || "Unknown application error";
        return {
          state: CdFxStateLevel.Error,
          data: null,
          message: msg,
  ...
  protected async emitHttpFx<T>(
    req: ICdRequest,
    opts: { spinner?: boolean; notify?: boolean } = {}
  ): Promise<CdFxReturn<T>> {
    try {
      ...
      if (!cdResp.app_state.success) {
        if (opts.notify)
          window.cdShell?.notify?.error(
            cdResp.app_state.info?.app_msg || "Login failed"
          );
        return {
          state: CdFxStateLevel.Error,
          data: null,
      ...
      if (opts.notify)
        window.cdShell?.notify?.success(
          `Welcome ${cdResp.data?.userName || "User"}`
        );
