import { isNode, isCapacitorNative } from "../../../../environment";
...
export class SQLiteStoreService {
    constructor(dbName = "cd-shell.db") {
        this.dbName = dbName;
        this.db = null;
    ...
    async save(key, data) {
        ...
        if (this.platform === "node") {
            const stmt = this.db.prepare(`INSERT INTO kv_store (key, value)
         VALUES (?, ?)
        ...
        if (this.platform === "capacitor") {
            await this.db.run(`INSERT OR REPLACE INTO kv_store (key, value) VALUES (?, ?)`, [key, JSON.stringify(data)]);
            return;
        }
    ...
    async get(key) {
        ...
        if (this.platform === "capacitor") {
            const result = await this.db.query(`SELECT value FROM kv_store WHERE key = ?`, [key]);
