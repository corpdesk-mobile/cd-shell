...
export class MetisMenuAdapter {
    ...
    async initialize(containerId, themeName) {
        ...
        if (!root)
            return;
        // 1. Mutate the HTML (assigns 'id="menu"')
        this.transformForMetisMenu(root);
        // 2. ASSERTION STEP: Verify the mutation succeeded before binding
        const menuRootEl = document.getElementById("menu");
        if (!menuRootEl) {
            // Log an error if the element is not found, indicating a failure in transformForMetisMenu
            console.error("[MetisMenuAdapter] FATAL: Failed to find element with ID 'menu' after transformation. MetisMenu will not bind.");
            return;
        }
        // 3. Initialize MetisMenu ONLY if the element is confirmed
        this.instance = new MetisMenu("#menu");
    }
    destroy() {
    ...
    transformForMetisMenu(root) {
        root.classList.remove("cd-menu-root");
        root.classList.add("metismenu");
        root.setAttribute("id", "menu"); // Set the ID for MetisMenu initialization
        root.querySelectorAll(".cd-menu-item").forEach((li) => {
            const link = li.querySelector(".cd-menu-link");
            const label = li.querySelector(".cd-menu-label");
            const route = li.getAttribute("data-route") || "#";
            const iconMetaEncoded = li.getAttribute("data-icon");
            const hasChildren = li.querySelector("ul");
            ...
            if (link && label) {
                // --- FIX 1: Clean up conflicting elements and structure ---
                // A. Remove the redundant toggle icon generated by renderMenuHtml (if present).
                const initialToggleIcon = link.querySelector(".cd-menu-toggle-icon");
                if (initialToggleIcon) {
                    initialToggleIcon.remove();
                }
                // B. Clear the link's content, we will rebuild it using the MetisMenu pattern.
                link.innerHTML = "";
                link.href = hasChildren ? "#" : `/${route}`; // Ensure href is correct for MetisMenu
                // --- FIX 2: Rebuild link content using MetisMenu structure ---
                // 1. Add Icon (menu-icon)
                if (iconMeta) {
                    const iconEl = this.createIconElement(iconMeta);
                    if (iconEl) {
                ...
                // 2. Add Label (reuse the existing label content)
                const labelSpan = document.createElement("span");
                labelSpan.classList.add("cd-menu-label");
                labelSpan.textContent = label.textContent || "";
                link.appendChild(labelSpan);
                // 3. Add Dropdown Arrow (menu-arrow) for MetisMenu CSS/JS
                if (hasChildren) {
                    link.classList.add("has-arrow"); // Required by MetisMenu
                    link.setAttribute("aria-expanded", "false");
                    const arrow = document.createElement("i");
                    // Use 'menu-arrow' for auto-alignment and 'fa-solid' for animation consistency
                    arrow.classList.add("menu-arrow", "fa-solid", "fa-chevron-right");
                    link.appendChild(arrow); // Appends the new, correctly styled arrow to the far right
                }
            }
            // Remove the neutral class, MetisMenu will handle this
            li.classList.remove("cd-menu-item");
        });
        // Remove neutral submenu class
        root.querySelectorAll("ul.cd-submenu").forEach((ul) => {
            ul.classList.remove("cd-submenu");
        });
    }
