...
export class MenuService {
    ...
    async beforeCreate(req, res) {
        ...
        let ret = false;
        const cdObjData = await svCdObj.createI(req, res, createIParams);
        if (cdObjData) {
    ...
    getMenu(req, res) {
        ...
        console.info('MenuService::getMenu/f:', f);
        const serviceInput = {
     ...
     * @returns
     */
    getAclMenu$(req, res, allowedModules, sessionDataExt) {
        console.info('MenuService::getAclMenu$()/allowedModules:', allowedModules);
        return allowedModules.modules$.pipe(mergeMap((m) => {
            return m.map((mod) => {
                console.info('MenuService::getAclMenu$()/mod:', mod);
                const moduleMenuData$ = this.getModuleMenu$(req, res, mod, sessionDataExt);
                console.info('MenuService::getAclMenu$()/moduleMenuData:', moduleMenuData$);
                return forkJoin({
                ...
                }).pipe(map(({ menu, modules }) => {
                    console.info('MenuService::getAclMenu$()/menu:', menu);
                    console.info('MenuService::getAclMenu$()/modules:', modules);
                    return menu; // menu now contains an array of nested menu trees
                }));
            });
        }), mergeMap((m) => {
            console.info('MenuService::getAclMenu$()/m:', m);
            return m.pipe(map((modules) => modules));
        }), bufferCount(allowedModules.modulesCount), map((bufferedMenus) => bufferedMenus.flat()));
    }
    getModuleMenu$(req, res, moduleData, sessionDataExt) {
        console.info('MenuService::getModuleMenu$()/moduleData:', moduleData);
    ...
    buildNestedMenu(rootMenuIds$, moduleMenuData$, sessionDataExt) {
        return rootMenuIds$.pipe(switchMap((rootMenuIds) => {
            const menuTrees$ = rootMenuIds.map((rootMenuId) => this.buildSingleMenuTree(rootMenuId, moduleMenuData$, sessionDataExt));
    ...
    //     );
    // }
    buildSingleMenuTree(rootMenuId, moduleMenuData$, sessionDataExt) {
        const cuid = sessionDataExt.currentUser.userId;
        return moduleMenuData$.pipe(map((menuData) => {
            console.log('MenuService::buildSingleMenuTree()/menuData1:', menuData);
    ...
    getRootMenuIds(moduleMenuData$, sessionDataExt) {
        return moduleMenuData$.pipe(map((menuData) => {
    ...
    getMenuItem(menuId$, moduleMenuData$, sessionDataExt) {
        this.b.logTimeStamp('MenuService::getMenuItem$/01');
        return moduleMenuData$.pipe(tap((m) => {
            console.log('MenuService::getMenuItem/m:', m);
            menuId$.pipe(map((mId) => {
                this.b.logTimeStamp('MenuService::getMenuItem$/02');
                return mId;
            }));
        }), mergeMap((mData) => forkJoin({
            menuData: of(mData),
            menuId: menuId$,
        }).pipe(defaultIfEmpty(null))), map((m) => {
            console.log('MenuService::getMenuItem/m:', m);
            if (m) {
                return m.menuData.filter((menuItem) => {
    ...
    getChildren(menuParentId, selectedMenu) {
