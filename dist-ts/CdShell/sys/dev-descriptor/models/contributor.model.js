...
export function getContributorsByNames(names, contributors) {
    const matchedVendor = contributors.vendor && names.includes(contributors.vendor.name)
        ? contributors.vendor
        : undefined;
    const matchedDevelopers = contributors.developers?.filter((developer) => names.includes(developer.name));
    const matchedCommunities = contributors.communities?.filter((community) => names.includes(community.name));
...
// Example Usage
const selectedContributors = getContributorsByNames(['John Doe', 'OpenSource Forum'], contributors);
console.log(selectedContributors);
const unknownContributors = getContributorsByNames(['Unknown Name'], contributors);
