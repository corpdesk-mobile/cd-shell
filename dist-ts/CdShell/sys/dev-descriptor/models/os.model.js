 ...
 */
export function getEnvironmentVariablesByContext(context) {
 ...
 */
export function getEnvironmentVariablesByNames(context, names) {
    return Object.entries(environmentVariables)
        .filter(([key, data]) => data.context.includes(context) && names.includes(key))
        .reduce((acc, [key, data]) => {
        acc[key] = data.value;
        return acc;
    }, {});
}
export function getOsByName(name, osStore) {
