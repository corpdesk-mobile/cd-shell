...
export class ProfileServiceHelper {
     ...
     */
    static async fetchProfile(req, res, si) {
    ...
    /**
       ...
       * @returns
       */
    static async modifyProfile(existingData, profileConfig) {
            ...
            if (firstKey === 'coopMembership' &&
                ...
                switch (action) {
                    ...
                    case 'update':
                        ...
                        break;
                    case 'delete':
                        updatedProfile = await this.deleteCoopRole(updatedProfile, remainingPath, value.coopId);
                        break;
                    case 'read':
                        await this.readCoopRole(updatedProfile, remainingPath, value.coopId);
                        break;
                    default:
     ...
     */
    static updatePermissions(profile, newValue, permissionType, idKey) {
        const permissionList = profile.fieldPermissions[permissionType];
        const existingIndex = permissionList.findIndex((permission) => permission[idKey] === newValue[idKey] &&
     ...
     */
    static applyJsonUpdate(profile, path, value) {
        let current = profile;
    ...
    //     return profile;
    // }
    static async createCoopRole(profile, path, newValue) {
        console.log('ProfileServiceHelper::createCoopRole()/profile:', profile);
        console.log('ProfileServiceHelper::createCoopRole()/newValue:', newValue);
    ...
    static updateCoopRole(profile, path, newValue) {
        const aclList = profile.coopMembership.acl;
        const targetAcl = aclList.find((acl) => acl.coopId === newValue.coopId);
        if (targetAcl) {
    ...
    static deleteCoopRole(profile, path, coopId) {
        const aclList = profile.coopMembership.acl;
        const index = aclList.findIndex((acl) => acl.coopId === coopId);
        if (index !== -1) {
    ...
    static readCoopRole(profile, path, coopId) {
        const aclList = profile.coopMembership.acl;
        const targetAcl = aclList.find((acl) => acl.coopId === coopId);
        if (targetAcl) {
    ...
    static validateAclItem(item) {
        const isValidCoopId = typeof item.coopId === 'number' || item.coopId === null;
        ...
        const isValidAclRole = item.aclRole === undefined ||
        ...
        return (isValidCoopId &&
            isValidCoopActive &&
            isValidCoopRole &&
            isValidAclRole &&
            isValidCoopMemberData);
    }
    static syncCoopMemberProfiles(modifiedProfile) {
        ...
        if ('coopMembership' in modifiedProfile) {
            ...
            modifiedProfile.coopMembership.memberData.forEach((member) => {
