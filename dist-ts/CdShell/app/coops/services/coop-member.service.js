...
import { UserModel, } from '../../../sys/cd-user/models/user.model.js';
...
export class CoopMemberService {
        ...
        for (let fVal of fValsArray) {
            ...
            if (await this.validateCreate(req, res)) {
                ...
                const serviceInput = {
                    ...
                    data: await this.b.getPlData(req),
                    docName: 'Create coop-member',
                    dSource: 1,
                };
                console.log('CoopMemberService::create()/req.post:', req.post);
                let respData = (await this.b.create(req, res, serviceInput));
                // Normalize respData to CdFxReturn<CoopMemberModel>
                if (!respData.state || !respData.data) {
                    return CD_FX_FAIL;
                }
    ...
    async validateCreate(req, res) {
        ...
        if (!valid) {
            console.info('coop/CoopMemberService::validateCreate()/Validation failed');
    ...
    async validateExistence(req, res, validationParams) {
        const promises = validationParams.map((param) => {
            ...
            return b.read(req, res, serviceInput).then((r) => {
                if (r.length > 0) {
                    console.info(`coop/CoopMemberService::validateExistence() - ${param.field} exists`);
                    return true;
                }
                else {
                    console.error(`coop/CoopMemberService::validateExistence() - Invalid ${param.field}`);
    ...
    async coopMemberExists(req, res, params) {
        const serviceInput = {
            serviceInstance: this,
            serviceModel: CoopMemberModel,
    ...
    //   //
    // }
    // async activateCoop(req: any, res: any) {
    //   try {
    ...
    //       await this.b.respond(req, res);
    //     }
    //     let pl: CoopMemberModel = await this.b.getPlData(req);
    //     console.log('CoopMemberService::activateCoop()/pl:', pl);
    //     const coopId = pl.coopId;
    //     const svSess = new SessionService();
    //     const sessionDataExtResult = await svSess.getSessionDataExt(
    //       req,
    //       res,
    //       true,
    //     );
    //     if (!sessionDataExtResult) {
    //       throw new Error('Session data could not be retrieved');
    //     }
    //     const sessionDataExt: ISessionDataExt = sessionDataExtResult;
    //     console.log(
    ...
    //   });
    // }
    // async updateI(req, res, serviceInput: IServiceInput<CoopStatModel>) {
    ...
    //  * @param q
    //  */
    // async getCoopMember(req, res, q?: IQuery ) {
    //   if (q === null) {
    ...
    //   }
    // }
    // async getCoopMemberProfile(req: any, res: any) {
    //   try {
    ...
    //   }
    // }
    // async validateGetCoopMemberProfile(req: any, res: any) {
    //   let ret = true;
    //   if (
    ...
    //   return ret;
    // }
    // async validateUpdateCoopMemberProfile(req: any, res: any) {
    //   let ret = true;
    ...
    //   return ret;
    // }
    // async getCoopMemberProfileI(req: any, res: any) {
    //   try {
    //     await this.setCoopMemberProfileI(req, res);
    ...
    //   }
    // }
    // async getI(req, res, q?: IQuery ): Promise<CoopMemberViewModel[]> {
    //   if (q === null) {
    //     q = this.b.getQuery(req);
    //   }
    //   console.log('CoopMemberService::getCoopMember/q:', q);
    ...
    //   }
    // }
    // async getCoopMemberCount(req: any, res: any) {
    ...
    //  * @param res
    //  */
    // async setCoopMemberProfileI(req: any, res: any) {
    ...
    //   }
    // }
    // async resetCoopMemberProfileI(req: any, res: any) {
     ...
     */
    addCoopRole(profile, coopId, newRole) {
     ...
     */
    updateCoopRole(profile, coopId, scope, updatedRole) {
        const memberMeta = profile.coopMembership.acl?.find((m) => m.coopId === coopId);
        if (memberMeta) {
     ...
     * @returns boolean indicating success or failure
     */
    deleteCoopRole(profile, coopId, scope) {
        const memberMeta = profile.coopMembership.acl?.find((m) => m.coopId === coopId);
        if (memberMeta) {
            const roleIndex = memberMeta.coopRole.findIndex((role) => role.scope === scope);
            if (roleIndex !== -1) {
